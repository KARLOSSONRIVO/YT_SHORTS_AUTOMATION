...
The same apostrophe also passes through two parser layers in FFmpeg's subtitle filter. Update the existing filter-path encoder to close the quoted path, preserve the apostrophe through both layers with three backslashes, and reopen the quoted path. Project titles, directories, URLs, scene files, and the final FFmpeg command remain unchanged.
...
3. The same title is preserved when its subtitle path passes through both FFmpeg filter parser layers.
